...
endpoint = st.sidebar.selectbox("Endpoints", ['Recently Listed', 'y00ts Overview', 'Test y00ts'])
st.header(f"y00ts Explorer - {endpoint}")


if endpoint == 'y00ts Overview':
    url_stats = requests.get('https://api-mainnet.magiceden.dev/v2/collections/y00ts/stats')
    ...
    st.image('https://img-cdn.magiceden.dev/rs:fill:400:400:0:0/plain/https://bafkreidc5co72clgqor54gpugde6tr4otrubjfqanj4vx4ivjwxnhqgaai.ipfs.dweb.link/')


if endpoint == 'Recently Listed':
    # floor_price_selector = st.sidebar.selectbox("Floor Price?", ['Below Floor', 'Above Floor'])
    url_listings = requests.get('https://api-mainnet.magiceden.dev/v2/collections/y00ts/listings?offset=0&limit=20')
    ...
    for yoot in listings_data:
        ...
        if price > floorPrice:
            ...
            st.metric(label=title, value=sol_price, delta=sol_diff, delta_color="neutral")
        else:
...
if endpoint == 'Test y00ts':
    ...
    for yoot in listings_data:
        ...
        #st.write(item_url)
        st.markdown(
            f"""<a href="https://magiceden.io/item-details/{mint_address}?name=y00t-%23{replace_2}">BUY NOW</a>""", unsafe_allow_html=True,
        )
...
# Magic Eden Codeimport requests
import streamlit as st
import json
import time
import datetime
